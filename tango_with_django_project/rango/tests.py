...
class Chapter8TemplateTests(TestCase):
    ...
    def test_template_usage(self):
        ...
        urls = [reverse('rango:about'), reverse('rango:add_category'),
                reverse('rango:add_page', kwargs={'category_name_slug': 'python'}),
                reverse('rango:show_category', kwargs={'category_name_slug': 'python'}), reverse('rango:index'), ]

        templates = ['rango/about.html', 'rango/add_category.html', 'rango/add_page.html', 'rango/category.html',
                     'rango/index.html', ]
    ...
    def test_title_blocks(self):
        ...
        mappings = {reverse('rango:about'): {
            ...
            reverse('rango:show_category', kwargs={'category_name_slug': 'python'}): {
                'full_title_pattern': r'<title>(\s*|\n*)Rango(\s*|\n*)-(\s*|\n*)Python(\s*|\n*)</title>',
                'block_title_pattern': r'{% block title_block %}(\s*|\n*){% if category %}(\s*|\n*){{ category.name }}(\s*|\n*){% else %}(\s*|\n*)Unknown Category(\s*|\n*){% endif %}(\s*|\n*){% (endblock|endblock title_block) %}',
                'template_filename': 'category.html'}, reverse('rango:index'): {
                'full_title_pattern': r'<title>(\s*|\n*)Rango(\s*|\n*)-(\s*|\n*)Homepage(\s*|\n*)</title>',
                'block_title_pattern': r'{% block title_block %}(\s*|\n*)Homepage(\s*|\n*){% (endblock|endblock title_block) %}',
    ...
    def test_for_links_in_base(self):
        ...
        look_for = ['<a href="{% url \'rango:add_category\' %}">Add a New Category</a>',
            '<a href="{% url \'rango:about\' %}">About</a>', '<a href="{% url \'rango:index\' %}">Index</a>', ]
